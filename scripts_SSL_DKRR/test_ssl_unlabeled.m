function filepath = test_ssl_unlabeled(data_name)
    [lambda, k_par, M, M_set, m, m_set, T, repeat, ntr, nts, ntr_ssl_set] = best_parameters_SSL_DKRR(data_name);
    ...
    nystroem_time = zeros(length(ntr_ssl_set), repeat);
    rf_time = zeros(length(ntr_ssl_set), repeat);
    falkon_time = zeros(length(ntr_ssl_set), repeat);
    labeled_training_time = zeros(length(ntr_ssl_set), repeat);
    ssl_training_time = zeros(length(ntr_ssl_set), repeat);

    nystroem_error = zeros(length(ntr_ssl_set), repeat);
    rf_error = zeros(length(ntr_ssl_set), repeat);
    falkon_error = zeros(length(ntr_ssl_set), repeat);
    labeled_test_error = zeros(length(ntr_ssl_set), repeat);
    ssl_test_error = zeros(length(ntr_ssl_set), repeat);

    for i=1:length(ntr_ssl_set)    
        ntr_ssl = floor(ntr_ssl_set(i));
        for j = 1:repeat
            ...
            Y_train_ssl(ntr_ssl+1:end) = Y_train_ssl(ntr_ssl+1:end) / m;
            % test
            ...
            if i==1
                labeled_training_time(:, j) = mean(dkrr_traing_times);
                labeled_test_error(:, j) = error_estimate(Y_test, mean(dkrr_Ypreds, 1)');
            end
    ...
    filepath = ['./results/ssl_unlabeled_', data_name, '_', datestr(now,30), '.mat' ]
    save(filepath);
end